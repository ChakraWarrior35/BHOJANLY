...
def add_to_cart(request, item_id):
    if request.method == 'POST':
        ...
        # Redirect to the cart page so users see the updated cart.
...
def buy(request):
    # Simple checkout: clear cart and thank the user
    if request.method == 'POST':
        request.session['cart'] = []
        messages.success(request, 'Purchase completed. Thank you!')
        return redirect('index')
    return redirect('cart')
